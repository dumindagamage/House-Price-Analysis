...
* **Pipeline Architecture:** To ensure reproducibility and prevent data leakage, all models were evaluated using a Scikit-Learn pipeline consisting of:
    * **Data Imputation:** Handling missing values via Mean/Median strategies.
    * **Feature Engineering:** Categorical encoding and feature transformation.
    * **Feature Scaling:** Normalizing numerical variables for consistent model interpretation.
...
This project solidified my ability to manage complex, real-world datasets and bridge the gap between classical statistics and modern machine learning. By building predictive pipelines and performing rigorous hyperparameter tuning, I gained a deeper appreciation for the challenges of working with raw data.

Developing the Streamlit dashboard significantly improved my data storytelling skills, teaching me how to translate technical metrics into actionable business insights for non-technical stakeholders. Furthermore, integrating AI tools into my workflow demonstrated how to leverage generative assistance responsibly to accelerate development without compromising code quality. This experience has prepared me for continuous learning in advanced statistical modeling and domain-specific analytics.
